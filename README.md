...
├── results/                     Pipeline outputs — gitignored, not committed; reproduce via pipeline
│   ├── awi_full.csv             AWI scores - 400 rows, player x match x phase
│   ├── pqi_full.csv             PQI scores - 400 rows, same structure
│   ├── combined_full.csv        AWI + PQI merged dataset - used by broadcast demo
│   ├── narratives.csv           Bedrock scouting narratives
│   ├── match_moments.csv        Top-5 pre-pass scan moments per match
│   └── per_match/               Raw per-match pipeline outputs
...
## Quick Start - Reproduce Results (AWS credentials required)

Result CSVs are not committed to this repository — they are derived outputs produced by the pipeline from the hackathon S3 data. To populate `results/` and then run the dashboard:

**Step 1 — Set up environment and activate AWS session**
```bash
# macOS / Linux
python3 -m venv .venv && source .venv/bin/activate
pip install -r requirements.txt
cp .env.example .env               # fill in AWS_PROFILE, HACKATHON_BUCKET, etc.
...
**Step 2 — Run the pipeline** (~15–20 min via SageMaker)
```bash
python pipelines/sagemaker_pipeline.py --action run
```

This produces `results/awi_full.csv`, `results/pqi_full.csv`, `results/combined_full.csv`, `results/match_moments.csv`, and (optionally) `results/narratives.csv` via Bedrock.
...
> **Note:** If your SSO token expires during Step 2 (~60 min TTL), re-run `source project_start.sh` and resume — completed phases are skipped automatically.
...
Requires `results/combined_full.csv` — produced by the pipeline (see Quick Start above).
...
Five Bundesliga matches provided by DFL via the hackathon S3 bucket. **No match data or derived result files are stored in this repository.** All CSVs under `results/` are pipeline outputs generated from the S3 Parquet source files and are gitignored. Run the pipeline (Steps 1–2 above) to reproduce them.
