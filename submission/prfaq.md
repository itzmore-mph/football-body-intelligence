...
AWI and PQI are not isolated inventions. Each metric has a direct conceptual ancestor in a proven system from another sport or domain. The table below maps each external system to the specific concept it pioneered and the precise way that concept is applied in this platform.

| External System | Concept | Application to AWI / PQI |
|---|---|---|
| **NFL Next Gen Stats** (AWS / NGS) | Spatial separation metrics — continuous tracking of receiver-defender distance and closing speed to quantify open-field advantage | PQI's **proximity sub-score** applies the same principle to pressing: distance to ball carrier is tracked frame-by-frame at 50 fps, and the score decays linearly from 100 (0 m) to 0 (≥5 m), mirroring how NGS quantifies separation as a continuous field rather than a binary "open/covered" flag |
| **NBA Second Spectrum** | Defensive matchup tracking — skeleton-level body orientation is used to determine which defender is "on" which offensive player and whether the defender's chest is facing the ball handler | PQI's **orientation sub-score** is the direct football equivalent: body yaw (derived from shoulder/hip vectors) is compared to the vector toward the ball carrier, and the angular deviation is penalised on a 0–100 scale. Second Spectrum uses the same angular-deviation logic to score defensive positioning quality |
| **Tennis Hawk-Eye** | Pose-based motion analysis — joint-angle trajectories are extracted from multi-camera skeleton data to quantify serve mechanics, split-step timing, and recovery stance | PQI's **stance sub-score** applies the same joint-angle approach to pressing: knee flexion is extracted from the TF15 3D skeleton and scored against the biomechanically optimal pressing angle (130°) using a Gaussian penalty. Hawk-Eye pioneered the idea that a single joint angle — measured continuously at high frame rate — can serve as a quality signal for athletic technique |
| **Medical gait analysis** (clinical biomechanics) | Knee-angle optimisation — clinical gait labs measure knee flexion during dynamic tasks (walking, stair descent, athletic cutting) and compare against population norms to identify injury risk or performance deficits | The 130° knee-flexion optimum in PQI's stance sub-score is derived directly from sports-medicine literature on athletic pressing stance (low centre of gravity, ready to change direction). The Gaussian scoring function — `100 × exp(−0.5 × ((knee_flex − 130) / 25)²)` — is the same mathematical form used in clinical gait deviation indices to penalise departure from a normative joint angle |
| **Rugby smart ball + pose fusion** (e.g., Gilbert Pulsar / Catapult) | Sensor fusion of ball-event data with player pose to compute tackle quality — body position at the moment of contact is combined with ball-carrier proximity and approach angle to score defensive technique | PQI is the football equivalent of tackle-quality scoring: it fuses three signals (body orientation, knee-flexion stance, proximity) at the moment of a press event, exactly as rugby systems fuse approach angle, body height, and contact force. The press-frame filter (≥10 consecutive frames within 5 m) is the football analogue of the contact-detection window in rugby sensor fusion |

**Why this matters for AWI specifically:** AWI's closest cross-domain ancestor is **cognitive load measurement in aviation and military simulation**, where head-scan rate (measured by eye-tracking or helmet-mounted sensors) is used as a real-time proxy for situational awareness. The +57% AWI spike in the 5 seconds before a pass is the football equivalent of the pre-decision scan burst documented in fighter-pilot studies — a direct, measurable signal that the player is actively building a mental model of the field before acting. TF15 skeleton data makes this measurement possible at matchday scale for the first time in team sports.
